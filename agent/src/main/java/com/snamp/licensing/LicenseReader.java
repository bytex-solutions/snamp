...
public final class LicenseReader {
    ...
    private static <T extends LicenseLimitations> T getLimitations(final String licensedObject, final Unmarshaller deserializer) throws JAXBException {
        final Element target = loadedLicense.getElementById(licensedObject);
        return (T)deserializer.unmarshal(target);
    }
    ...
    public static <T extends LicenseLimitations> T getLimitations(final Class<T> limitationsDescriptor, final Activator<T> fallback){
        ...
        if(limitationsDescriptor == null) return fallback.newInstance();
            ...
            result = limitationsDescriptor.isAnnotationPresent(XmlRootElement.class) ?
                LicenseReader.<T>getLimitations(limitationsDescriptor.getAnnotation(XmlRootElement.class).name(), context.createUnmarshaller()):
                LicenseReader.<T>getLimitations(limitationsDescriptor.getSimpleName().replaceFirst("LicenseLimitations", ""), context.createUnmarshaller());
