...
export class WebSocketClient  {
  ...
  constructor(private url:string, private options?:any, private config?: WebSocketConfig  ) {
  ...
  connect(force:boolean = false) {
    ...
    if (force || !this.socket || this.socket.readyState !== this.readyStateConstants.OPEN) {
      self.socket = this.options ? new WebSocket(this.url, this.options) : new WebSocket(this.url);
