...
export class WebSocketClient {
    ...
    constructor(private url: string, private protocols?: Array<string>, private config?: WebSocketConfig, private binaryType?: BinaryType) {
        ...
        this.config = config || {initialTimeout: 9000, maxTimeout: 300000, reconnectIfNotNormalClose: true};
    ...
    fireQueue() {
        ...
        while (this.sendQueue.length && this.socket.readyState === this.readyStateConstants.OPEN) {
            ...
            this.socket.send(
                WebSocketClient.Helpers.isString(data.message) ? data.message : JSON.stringify(data.message)
            );
    ...
    onMessage(callback, options?) {
        if (!WebSocketClient.Helpers.isFunction(callback)) {
