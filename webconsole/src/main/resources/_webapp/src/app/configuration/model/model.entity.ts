export abstract class Entity {
    ...
    private static decapitalizeFirstLetter(object: string): string {
    ...
    getName() {
        ...
        return (results && results.length > 1) ? Entity.decapitalizeFirstLetter(results[1]) : "";
    }
    ...
    public static containsInArray(element:Entity, array:Entity[]):boolean {
        let _found:boolean = false;
        for (let i = 0; i < array.length; i++) {
            if (array[i].guid == element.guid) {
                _found = true;
                break;
            }
        }
