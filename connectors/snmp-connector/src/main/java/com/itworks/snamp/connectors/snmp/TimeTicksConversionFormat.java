...
enum TimeTicksConversionFormat {
    ...
    public SMITypeProjection<TimeTicks, ?> createTypeProjection(){
        switch (this){
            case TEXT: return new SMITypeProjection<TimeTicks, String>(TimeTicks.class, String.class) {
                @Override
                protected String convertFrom(final TimeTicks value) throws IllegalArgumentException {
                    return value.toString();
                }
            };
            default: return new SMITypeProjection<TimeTicks, Long>(TimeTicks.class, Long.class) {
                @Override
