    ...
    public String execute() throws Exception {
        ...
        if (supervisorRef.isPresent()) {
            try (final SupervisorClient client = supervisorRef.get()) {
                policy.evaluate(new ScalingPolicyDebuggerContext(client));
            }
            return "Scaling policy executed successfully";
        } else
            return String.format("Resource group %s doesn't exist", groupName);
    }
}
