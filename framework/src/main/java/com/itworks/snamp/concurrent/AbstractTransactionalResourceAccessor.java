...
public abstract class AbstractTransactionalResourceAccessor<R, S, C> extends AbstractConcurrentResourceAccessor<R> {
     ...
     */
    public final boolean prepare(final Supplier<C> changesetProvider, final ConsistentAction<R, S> snapshotProvider){
        ...
        try{
            ...
            snapshot.set(new PreparationData<>(snapshotProvider.invoke(getResource()), changesetProvider.get()));
            return true;
        }
        finally {
            wl.unlock();
    ...
    public final boolean commit(final TransactionPhaseProcessor<R, C, Boolean> committer){
        ...
        try{
            if(snapshot.get() == null) return false;
            else if(committer.invoke(getResource(), snapshot.get().changeset)){
                snapshot.set(null);
                return true;
            }
            else return false;
    ...
    public final boolean rollback(final TransactionPhaseProcessor<R, S, Boolean> restorer){
        ...
        try{
            if(snapshot.get() == null) return false;
            else if(restorer.invoke(getResource(), snapshot.get().snapshot)){
                snapshot.set(null);
                return true;
            }
            else return false;
